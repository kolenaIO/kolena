...
def compute_test_sample_metrics(
    ...
    return [
        (ts_gt_inf[0], compute_image_metrics(bbox_matches, thresholds))
        for ts_gt_inf, bbox_matches in zip(results, all_bbox_matches)
    ]


# TODO: compute_aggregate_label_metrics - nested metrics by class
...
def compute_test_case_metrics_and_plots(
    tc_gts: List[GroundTruth],
    tc_infs: List[Inference],
    ...
    plots: List[Plot] = compute_pr_f1_plots(all_bbox_matches, "baseline")
    baseline_pr_plot: Curve = next((curve for curve in plots[1].curves if curve.label == "baseline"), None)
    confusion_matrix = compute_confusion_matrix_plot(all_bbox_matches)

    if confusion_matrix is not None:
        plots.append(confusion_matrix)

    # TODO: compute nested metrics per class
    # labels: List[str] = confusion_matrix.labels
    # per_class_metrics: List[ClassMetricsPerTestCase] = []
    # for label in labels:
    #     metrics_per_class = compute_aggregate_label_metrics(all_bbox_matches, thresholds, label)
    #     per_class_metrics.append(metrics_per_class)

    tp_count = fp_count = fn_count = confused_count = 0
    for match in all_bbox_matches:
        for _, inf in match.matched:
        ...
        fn_count += len(match.unmatched_gt)

        for _, inf in match.unmatched_gt:
            if inf is not None and inf.score >= thresholds[inf.label]:
                confused_count += 1
