...
def initialize(
    ...
    **kwargs: Any,
    ...
    !!! note
        As of version 0.29.0: the entity param is no longer needed; ``initialize(entity, token)`` is **deprecated**
        and replaced by ``initialize(token)``.

    Args:
        api_token: provided API token. This token is a secret and should be treated with caution
        verbose: optionally configure client to run in verbose mode, providing more information about execution. All
    ...
    Raises:
        ...
        InputValidationError: the provided combination or number of args is not valid
    """
