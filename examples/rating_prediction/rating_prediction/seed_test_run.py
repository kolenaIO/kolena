...
import pandas as pd
from rating_prediction.evaluator import evaluate_rating_prediction
from rating_prediction.workflow import ThresholdConfiguration
from rating_prediction.workflow import Inference
from rating_prediction.workflow import Model
...
def seed_test_run(model_name: str, test_suite_names: List[str]) -> None:
    ...
    model_descriptor = f"{model_name} [{DATASET}] :: ml-small"
    ...
    configurations = [
        ThresholdConfiguration(rating_threshold=3.5, k=2),
    ]

    for test_suite_name in test_suite_names:
        test_suite = TestSuite.load(test_suite_name)
        print(f"Test Suite: {test_suite}")
        test(model, test_suite, evaluate_rating_prediction, configurations, reset=True)
