...
class ActivationMapUploader:
    ...
    def submit(self, datapoint_names: List[str]) -> None:
        info("generating and uploading activation maps...")
        for _ in tqdm(
            self.pool.map(self.process_activation_map, datapoint_names),
            total=len(datapoint_names),
        ):
            pass
        info("finished uploading activation maps")

    def process_activation_map(self, datapoint_name: str) -> None:
        prob_array = download_binary_array(self.inf_locator_prefix + f"{datapoint_name}_person.npy")
        activation_map = create_bitmap(prob_array)
        upload_image_buffer(self.map_locator_prefix + f"{datapoint_name}.png", activation_map)
