...
# limitations under the License.
# type: ignore
# Copied from https://github.com/open-mmlab/mmdetection3d
    ...
    for m, current_class in enumerate(current_classes):
        for idx_l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                # MODIFIED BY KOLENA: removed it to keep the original order for plotting PR curves
                # for i in range(len(thresholds)):
...
def do_eval(gt_annos, dt_annos, current_classes, min_overlaps, eval_types=["bbox", "bev", "3d"]):
    """
    MODIFIED BY KOLENA: exposing returned dict from eval_class
    """
    ...
    if "bbox" in eval_types:
        ...
        mAP11_bbox = get_mAP11(ret["bbox"]["precision"])
        mAP40_bbox = get_mAP40(ret["bbox"]["precision"])
        if "aos" in eval_types:
            mAP11_aos = get_mAP11(ret["bbox"]["orientation"])
            mAP40_aos = get_mAP40(ret["bbox"]["orientation"])

    mAP11_bev = None
    mAP40_bev = None
    if "bev" in eval_types:
        ret["bev"] = eval_class(gt_annos, dt_annos, current_classes, difficultys, 1, min_overlaps)
        mAP11_bev = get_mAP11(ret["bev"]["precision"])
        mAP40_bev = get_mAP40(ret["bev"]["precision"])

    mAP11_3d = None
    mAP40_3d = None
    if "3d" in eval_types:
        ret["3d"] = eval_class(gt_annos, dt_annos, current_classes, difficultys, 2, min_overlaps)
        mAP11_3d = get_mAP11(ret["3d"]["precision"])
        mAP40_3d = get_mAP40(ret["3d"]["precision"])
    return (mAP11_bbox, mAP11_bev, mAP11_3d, mAP11_aos, mAP40_bbox, mAP40_bev, mAP40_3d, mAP40_aos, ret)
...
def kitti_eval(gt_annos, dt_annos, current_classes, eval_types=["bbox", "bev", "3d"]):
    """
    MODIFIED BY KOLENA - expose precisions and recalls returned by do_eval
    KITTI evaluation.
    Args:
    ...
    compute_aos = False
    # MODIFIED BY KOLENA - commented out the section below as it is possible to have an empty GT.
    # pred_alpha = False
    ...
    mAP11_bbox, mAP11_bev, mAP11_3d, mAP11_aos, mAP40_bbox, mAP40_bev, mAP40_3d, mAP40_aos, pr = do_eval(
        gt_annos,
        dt_annos,
        current_classes,
        min_overlaps,
    ...
    ret_dict = {}
    ret_dict["classes"] = [class_to_name[cls] for cls in current_classes]
    ...
    # MODIFIED BY KOLENA: add precisions and recalls to the returned dict
    for eval in eval_types:
        if eval == "aos":
            continue
        for class_id, class_name_id in enumerate(current_classes):
            for diff_id, diff in enumerate(difficulty):
                class_name = class_to_name[class_name_id]
                precisions = pr[eval]["precision"][class_id, diff_id, 0, :]
                recalls = pr[eval]["recall"][class_id, diff_id, 0, :]

                # trim trailing zeros
                nonzero_mask = recalls != 0
                trailing_zero_index = len(nonzero_mask) - nonzero_mask[::-1].argmax()
                ret_dict[f"{eval}_{class_name}_{diff}_precisions"] = precisions[:trailing_zero_index]
                ret_dict[f"{eval}_{class_name}_{diff}_recalls"] = recalls[:trailing_zero_index]
