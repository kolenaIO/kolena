...
def test__register(fr_test_data: TestData) -> None:
    # image registered in fr_test_data fixture, register should not have impact
    with TestImages.register() as registrar:
        for record in fr_test_data.images:
            registrar.add(
                record[0],
                record[1],
                record[2],
...
def test__load__augmented(
    ...
    df_test_image_1 = TestImages.load(include_augmented=True, data_source=data_source_1)
    df_test_image_2 = TestImages.load(include_augmented=True, data_source=data_source_2)
    df_test_image_augmented_source = fr_augmented_images_expected_df[
        fr_augmented_images_expected_df["data_source"] == data_source_1
    ]

    df_test_image = pd.concat([df_test_image_1, df_test_image_2])
    df_test_image_registered = df_test_image[df_test_image["data_source"].isin(fr_images_df["data_source"])]
    assert len(df_test_image_registered) == len(fr_images_df) + len(fr_augmented_images_df)
    df_test_image_loaded_augmented = df_test_image_registered.dropna(subset=["original_locator"])
    pandas.testing.assert_frame_equal(df_test_image_loaded_augmented, fr_augmented_images_expected_df)

    df_test_image_loaded_augmented_source = df_test_image.dropna(subset=["original_locator"])
    pandas.testing.assert_frame_equal(df_test_image_loaded_augmented_source, df_test_image_augmented_source)
