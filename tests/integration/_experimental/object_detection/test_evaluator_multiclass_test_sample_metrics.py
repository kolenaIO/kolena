...
EXPECTED_COMPUTE_TEST_SAMPLE_METRICS: Dict[Tuple[str, str], List[Tuple[TestSample, TestSampleMetrics]]] = {
    ...
    ("Threshold: Fixed(0.5) by class, IoU: 0.5, confidence ≥ 0.0", "iou=0.5 and diff labels but 0.5 confidence"): [
        (
            ...
            TestSampleMetrics(
                TP=[],
                FP=[ScoredLabeledBoundingBox(top_left=(2.0, 1.0), bottom_right=(5.0, 4.0), label="b", score=0.5)],
                FN=[LabeledBoundingBox(top_left=(1.0, 1.0), bottom_right=(4.0, 4.0), label="a")],
                Confused=[ScoredLabeledBoundingBox(top_left=(2.0, 1.0), bottom_right=(5.0, 4.0), label="b", score=0.5)],
                count_TP=0,
                count_FP=1,
                count_FN=1,
                count_Confused=1,
    ...
    ("Threshold: Fixed(0.5) by class, IoU: 0.5, confidence ≥ 0.0", "iou=0.5 and diff labels but 0.99 confidence"): [
        (
            ...
            TestSampleMetrics(
                TP=[],
                FP=[ScoredLabeledBoundingBox(top_left=(2.0, 1.0), bottom_right=(5.0, 4.0), label="b", score=0.99)],
                FN=[LabeledBoundingBox(top_left=(1.0, 1.0), bottom_right=(4.0, 4.0), label="a")],
                Confused=[
                    ScoredLabeledBoundingBox(top_left=(2.0, 1.0), bottom_right=(5.0, 4.0), label="b", score=0.99),
                ],
                count_TP=0,
                count_FP=1,
                count_FN=1,
...
def test__prebuilt__object__detection__multiclass__compute__test__sample__metrics__all() -> None:
    ...
    expected_test_sample_metrics = {
        details[1]: results
        for details, results in EXPECTED_COMPUTE_TEST_SAMPLE_METRICS.items()
        if details[0] == config_name
    }
    expected = [pair for _, results in expected_test_sample_metrics.items() for pair in results]
    expected_tests = list(expected_test_sample_metrics.keys())
    inferences = [data for test_name in expected_tests for data in TEST_DATA[test_name]]
    eval = ObjectDetectionEvaluator(configurations=[config])
    result = eval.compute_test_sample_metrics(
        test_case=TEST_CASE,
