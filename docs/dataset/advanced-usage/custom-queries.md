...
|                          | `split_part`  | `split_part(@datapoint.a, <delimter>, <index>)`     | Splits a given string by a character and returns the requested part.<br>The index is 1-indexed, not 0-indexed. For example, `split_part('a/b/c', '/', 2)` returns `b`, not `c`.                                                                                                                  |
