...
Kolena will attempt to infer common `id_field`s (eg. `locator`, `text`) based on what is present in the dataset during
import.
...
| `file_type`| Optional field indicating the type of file (e.g., `jpg`, `mp4`). Used to assist in the inference of the data type.                          |
